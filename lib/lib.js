...
function rpc(request, callback) {
  return new Promise((resolve, reject) => {
    ...
    const auth = new Buffer(self.user + ':' + self.pass).toString('base64');
    ...
    const req = this.protocol.request(options, function (res) {
      ...
      res.on('end', function () {
        ...
        if (!Array.isArray(parsedBuf) && parsedBuf.error !== null) {
          if (typeof callback === 'function') callback(parsedBuf.error);
          else reject(parsedBuf.error);
          return;
        }
        if (typeof callback === 'function') callback(parsedBuf.error, parsedBuf);
